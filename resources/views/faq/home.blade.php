...
@section('main-content')
    ...
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li @if ((count($errors) > 0) || isset($faq) || count($faqs) == 0) class="" @else class="active" @endif >
                <a href="#allFaq" data-toggle="tab">
                    {{ trans('common.all') . ' '. trans('common.faq')}}</a>
            </li>
            <li @if ((count($errors) > 0) || isset($faq) || count($faqs) == 0) class="active" @else class="" @endif ><a
                        href="#faq"
                        data-toggle="tab">{{ $title }}</a>
            </li>
            <!-- <li><a href=""
                   data-toggle="modal"
                   data-target="#upload_template">{{ trans('common.upload'). ' ' .trans('common.template') }}</a>
            </li> -->
        </ul>
        <div class="tab-content no-padding">
            <!-- ************** general *************-->
            <div @if ((count($errors) > 0) || isset($faq) || count($faqs) == 0) class="tab-pane" @else class="tab-pane active"
                 @endif id="allFaq">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" data-toggle="modal"
                                       data-target="#faq_detail_{{ $list->id }}"
                                ...
                                <td>
                                    {!! Form::open(['method' => 'DELETE', 'route' => ['faq-setting.destroy', $list->id], 'class' => 'delete-form']) !!}
                                    <a href="#" data-toggle="modal"
                                       data-target="#faq_detail_{{ $list->id }}" class='btn btn-info btn-xs'
                                       data-toggle='tooltip' data-placement='top'
                                       title='{{ trans('common.detail') }}'><i
                                                class='fa fa-list-alt'></i></a>
                                    {!! btn_edit("faq-setting/$list->id/edit") !!}
                                    {!! btn_delete_submit() !!}
                                    {!! Form::close() !!}
                                </td>
                            </tr>
            ...
            <div @if ((count($errors) > 0) || isset($faq) || count($faqs) == 0) class="tab-pane active" @else class="tab-pane"
                 @endif id="faq">

                @if( !isset($faq) )
                    {!! Form::open(['url' => "faq-setting",'id'=>'faq', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'No', 'files' => true]) !!}

                    @include('faq._form',['submit_button' => 'Submit'])

                    {!! Form::close() !!}
                @else
                    {!! Form::model($faq,['method' =>'PUT', 'url' => ["faq-setting",$faq->id],'id'=>'faq', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'Yes', 'files' => true]) !!}

                    @include('faq._form',['submit_button' => 'Update'])
    ...
    <div class="modal fade" id="upload_faq" tabindex="-1" role="dialog"
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"
                        id="myModalLabel">{{ trans('common.upload'). ' ' .trans('common.faq') }}</h4>
                </div>
                <div class="modal-body">
                    <div class="callout callout-info">
                        Please see <a class="text-warning" href="/customer/assets/files/example-faq.zip" data-original-title=""
                    ...
                    {!! Form::open(['url' => "upload_faq", 'id'=>'upload_faq_form', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'No', 'files' => true]) !!}
                        <div class="form-group">
                            <label for="CustomerEmailFaq_archive" class="required">Archive file <span
                                        class="required">*</span></label>
                            <input class="form-control" placeholder="Archive file"
                                    name="upload_faq" id="CustomerEmailFaq_archive"
                                    type="file">
                            <p class="text-danger" id="upload_faq_error"></p>
                        </div>
    ...
    @foreach($faqs as $list)
        <div class="modal fade" id="faq_detail_{{ $list->id }}" tabindex="-1" role="dialog"
             aria-labelledby="myModalLabel">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                    aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title"
                            id="myModalLabel">{{ $list->name . ' ' . trans('common.faq') }}</h4>
                    </div>
                    <div class="modal-body">
                        {!! $list->faq !!}
                    </div>
    ...
    <script>
        function submit_form(e) {
            e.preventDefault();
            var formData = new FormData($('#upload_faq_form')[0]);
            var url = "{{ url('upload_faq') }}";
        ...
        function useMedia() {
            ...
            $('#faq_body').summernote('focus');
            $('#select_media :selected').each(function (i, selected) {
                $('#faq_body').summernote('editor.insertImage', $(selected).attr('data-img-src'));
            });
            $('#select_media').val('');
                ...
                success: function (data) {
                    ...
                    if (data.success) {
                        if ($(that).attr('value') == 'Upload & Import') {
                            ...
                            $('#allMediaDiv').click();
                            $('#faq_body').summernote('focus');
                            $('#faq_body').summernote('editor.insertImage', img_base_path + '/' + data.media_info.path);
                        } else {
