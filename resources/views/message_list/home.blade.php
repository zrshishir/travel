...
@section('main-content')
    ...
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li @if ((count($errors) > 0) || isset($messageList)) class="" @else class="active" @endif >
                <a href="#allMessageList" data-toggle="tab">
                    {{ $pageName }}</a>
            </li>
            {{--            @if(Auth::user()->role == 'superadmin' || (isset($template) && ($template->id == Auth::id())))--}}
            <li @if ((count($errors) > 0) || isset($messageList)) class="active" @else class="" @endif ><a
                        href="#template"
        ...
        <div class="tab-content no-padding">
            <!-- ************** general *************-->
            <div @if ((count($errors) > 0) || isset($messageList)) class="tab-pane" @else class="tab-pane active"
                 @endif id="allMessageList">
                ...
                <table class="table table-hover" id="dataTables">
                    ...
                    @if(count($messageLists ) > 0)
                        @foreach($messageLists  as $key =>$list)
                            <tr>
                                <td>{!! $key+1 !!}</td>
                                <td>
                                    <a href="{{ url('message-list/'.$list->id) }}"
                                       data-toggle='tooltip' data-placement='top' title='{{ trans('common.message') }}'>{{ $list->name }}
                                    </a>
                                </td>
                                <td>

                                    {!! Form::open(['method' => 'DELETE', 'route' => ['message-list.destroy', $list->id], 'class' => 'delete-form']) !!}
                                    {{--{!! btn_show("template/$list->id")  !!}--}}
                                    <a href="{{ url('message-list/'.$list->id) }}" class='btn btn-info btn-xs'
                                       data-toggle='tooltip' data-placement='top' title='{{ trans('common.message') }}'><i
                                                class='fa fa-list-alt'></i></a>
                                    @if($list->id != 1)
                                        {!! btn_edit("message-list/$list->id/edit") !!}
                                        {!! btn_delete_submit() !!}
                                    @endif
                                        {!! Form::close() !!}
                                </td>
            ...
            <div @if ((count($errors) > 0) || isset($messageList)) class="tab-pane active" @else class="tab-pane"
                 @endif id="template">

                @if( !isset($messageList) )
                    {!! Form::open(['url' => "message-list",'id'=>'template', 'class'=>'form-horizontal', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'No', 'files' => true]) !!}

                    @include('message_list._form',['submit_button' => 'Submit'])

                    {!! Form::close() !!}
                @else
                    {!! Form::model($messageList,['method' =>'PUT', 'url' => ["message-list",$messageList->id],'id'=>'template', 'class'=>'form-horizontal', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'Yes', 'files' => true]) !!}

                    @include('message_list._form',['submit_button' => 'Update'])
