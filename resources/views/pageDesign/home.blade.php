...
@section('main-content')
    ...
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li @if(!isset($template)) class="active" @else class="" @endif> 
                <a href="#allTemplate" data-toggle="tab">
                    {{ trans('common.all') . ' '. trans('common.page')}}</a>
            </li>
            @if(isset($template))
        ...
        <div class="tab-content no-padding">
            <!-- ************** general *************-->
            <div  @if(!isset($template)) class="tab-pane active" @else class="tab-pane" @endif id="allTemplate">
                        ...
                        @foreach($templates  as $key =>$list)
                            <tr>
                                ...
                                <td>
                                 ...
                                 <td>
                                    @if($list->show_hide == 1)
                                        <button class="btn btn-info btn-xs" id="showHide_{{ $list->id }}" onClick="showHide({{ $list->id }})" value="{{ $list->show_hide }}"><i class="fa fa-check" ></i></button>
                                    @else
                                        <button class="btn btn-info btn-xs" id="showHide_{{ $list->id }}" onClick="showHide({{ $list->id }})" value="{{ $list->show_hide }}"><i class="fa fa-square-o" ></i></button>
                                    @endif
                                </td>
                                ...
                                <td>
                                    ...
                                    {!! btn_edit("page-design/$list->id/edit") !!}
                                    <!-- {!! btn_delete_submit() !!} -->
                                    {!! Form::close() !!}
                                </td>
                            </tr>
            ...
            <div @if ((count($errors) > 0) || isset($template) || count($templates) == 0) class="tab-pane active" @else class="tab-pane"
                ...
                @if( isset($template) )
                {!! Form::model($template,['method' =>'PUT', 'url' => ["page-design",$template->id],'id'=>'template', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'Yes', 'files' => true]) !!}

                @include('pageDesign._form',['submit_button' => 'Update'])
    ...
    <script>
        ...
        function showHide($id){
            ...
            $.ajax({
                ...
                method: 'GET',
                dataType: 'json',
                data: {'showHide': showHide, 'pageId' : $id},
                cache: false,
                success: function (data) {
                    window.location.href = '{{ url('page-design') }}';
                }
            });
        }
