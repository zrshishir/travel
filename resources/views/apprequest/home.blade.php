...
@section('main-content')
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li class="active" ><a href="#all_users" data-toggle="tab">All Requests</a></li>
             @if (Auth::user()->role == "user" )
                <li @if (Auth::user()->role == "user" && count($allRequests) == 0) class="" @else class="" @endif ><a href="#manage_users"
                    ...
                    @if(count($allRequests) > 0)
                        @foreach($allRequests as $key =>$user)
                            ...
                            <td><a href="" data-toggle="modal" data-target="#user_detail_{{ $user->id }}"
                                ...
                                @if((Auth::user()->role == 'hr' && $user->status == 0) || (Auth::user()->role == 'managing' && $user->status == 1))
                                <td>
