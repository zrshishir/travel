...
@section('main-content')
    ...
    <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs">
            <li @if ((count($errors) > 0) || isset($newsletter)) class="" @else class="active" @endif >
                <a href="#allTemplate" data-toggle="tab">
                    {{ trans('common.all') . ' '. trans('common.newsletter')}}</a>
            </li>
            <li @if ((count($errors) > 0) || isset($newsletter)) class="active" @else class="" @endif ><a
                        href="#template"
        ...
        <div class="tab-content no-padding">
            <!-- ************** general *************-->
            <div @if ((count($errors) > 0) || isset($newsletter)) class="tab-pane" @else class="tab-pane active"
                ...
                <table class="table table-hover" id="dataTables">
                    ...
                    @if(count($newsletters ) > 0)
                        @foreach($newsletters  as $key =>$list)
                            <tr>
                                <td>{!! $key+1 !!}</td>
                                <td>
                                    <a href="{{ 'newsletter/'.$list->id }}" target="_blank"
                                       data-toggle='tooltip' data-placement='top' title='{{ trans('common.detail') }}'>{{ $list->name }}
                                    </a>
                                </td>

                                <td>
                                    {!! Form::open(['method' => 'DELETE', 'route' => ['newsletter.destroy', $list->id], 'class' => 'delete-form']) !!}
                                    <a href="{{ 'newsletter/'.$list->id }}" target="_blank" class='btn btn-info btn-xs'
                                       data-toggle='tooltip' data-placement='top' title='{{ trans('common.detail') }}'><i
                                                class='fa fa-list-alt'></i></a>
                                    {!! btn_edit("newsletter/$list->id/edit") !!}
                                    {!! btn_delete_submit() !!}
                                    {!! Form::close() !!}
                                </td>
                            </tr>
            ...
            <div @if ((count($errors) > 0) || isset($newsletter)) class="tab-pane active" @else class="tab-pane"
                 @endif id="template">

                @if( !isset($newsletter) )
                    {!! Form::open(['url' => "newsletter",'id'=>'template', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'No', 'files' => true]) !!}

                    @include('newsletter._form',['submit_button' => 'Submit'])

                    {!! Form::close() !!}
                @else
                    {!! Form::model($newsletter,['method' =>'PUT', 'url' => ["newsletter",$newsletter->id],'id'=>'template', 'role' => 'form', 'data-toggle' => 'form-ajax', 'data-url' => 'Yes', 'files' => true]) !!}

                    @include('newsletter._form',['submit_button' => 'Update'])
